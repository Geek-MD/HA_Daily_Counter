...
- ✅ **CI mypy failure on `domain=DOMAIN` class keyword**: `mypy.ini` had `python_version = 3.12` but CI runs Python 3.11. Reverted to `python_version = 3.11` and restored the required `# type: ignore[call-arg]` suppression on the `FlowHandler` class declaration (mypy 3.11 does not model HA's `__init_subclass__` `domain` keyword).
